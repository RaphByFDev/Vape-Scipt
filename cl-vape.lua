	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Tu ~b~commence ~c~ a utiliser  ta puff.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Tu as deja ta Puff en main.")
			end
		else
			ShowNotification("~r~Tu ne peux pas faire ça en voiture.")
		end
	else
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Tu ~b~arrete ~c~ d'utiliser  ta puff.")
	else
		ShowNotification("~r~Tu n'as pas ta puff.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("~r~Tu dois avoir la puff en main.")
	end
end)
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Maintiens ~b~\"E\"~c~ pour tirer une taff.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Maintiens ~b~\"G\"~c~ pour reset ta position.")
			end
		end
	  Wait(1)
	end
